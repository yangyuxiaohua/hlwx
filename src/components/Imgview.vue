...
export default {
      ...
      vm.$nextTick(function() {
        ...
        vm.init().then(() => {
          vm.$nextTick(function() {
            ...
            if (
              vm.computedHeight / vm.computedWidth <
              vm.imgHeight / vm.imgWidth
            ) {
            ...
            vm.styleObjectImg.backgroundSize = cover;
            vm.styleObjectImg.backgroundPosition =
              (vm.computedWidth - vm.imgWidth) / 2 +
              "px " +
              (vm.computedHeight - vm.imgHeight) / 2 +
              "px";
          });
        });
      });
  ...
  mounted() {
    ...
    vm.init().then(() => {
      vm.$nextTick(function() {
        ...
        vm.styleObjectImg.left = "0px";
        vm.styleObjectImg.top = "0px";
        vm.styleObjectImg.backgroundSize =
          vm.imgWidth + "px " + vm.imgHeight + "px";
        vm.styleObjectImg.backgroundPosition =
          (vm.computedWidth - vm.imgWidth) / 2 +
          "px " +
          (vm.computedHeight - vm.imgHeight) / 2 +
  ...
  methods: {
    ...
    onScroll(e) {
      var e = e || window.event;
      let backgroundSizeX = parseFloat(
        this.styleObjectImg.backgroundSize.split(" ")[0]
      );
      let backgroundSizeY = parseFloat(
        this.styleObjectImg.backgroundSize.split(" ")[1]
      );
      let positionX = parseFloat(
        this.styleObjectImg.backgroundPosition.split(" ")[0]
      );
      let positionY = parseFloat(
        this.styleObjectImg.backgroundPosition.split(" ")[1]
      );
      if (e.deltaY < 0) {
        this.styleObjectImg.backgroundSize =
          backgroundSizeX * 1.1 + "px " + backgroundSizeY * 1.1 + "px";
    ...
    rotateLeft() {
      let vm = this;
      let backgroundSizeX = parseInt(
        this.styleObjectImg.backgroundSize.split(" ")[0]
      );
      let backgroundSizeY = parseInt(
        this.styleObjectImg.backgroundSize.split(" ")[1]
      );
      let positionX = parseInt(
        vm.styleObjectImg.backgroundPosition.split(" ")[0]
      );
      let positionY = parseInt(
        vm.styleObjectImg.backgroundPosition.split(" ")[1]
      );
      ...
      if (r % 180 == 90) {
        ...
        vm.styleObjectImg.left =
          Math.abs(parseInt(vm.computedHeight) - parseInt(vm.computedWidth)) /
            2 +
          "px";
        vm.styleObjectImg.top =
          (vm.computedHeight - vm.computedWidth) / 2 + "px";
        ...
        vm.styleObjectImg.backgroundSize =
          vm.imgWidth + "px " + vm.imgHeight + "px";
        vm.styleObjectImg.backgroundPosition =
          (vm.computedHeight - vm.imgWidth) / 2 +
          "px " +
          (vm.computedWidth - vm.imgHeight) / 2 +
          "px";
      } else {
        vm.styleObjectImg.width = vm.computedWidth + "px";
        vm.styleObjectImg.height = vm.computedHeight + "px";
